#' Rboretum Batch Site and Node Support Plotter
...
#'   \item A named, rooted multiPhylo object
#' }
#' @param tree_support  OPTIONAL: Output trom getTreesupport() or batch_getTreeSupport run on same 'tree' as above
...
batch_supportTreePlotter <- function(tree,tree_support,support_scales,node_alpha,node_color,legend_shape_size,legend_font_size,legend_title_size,branch_length,branch_weight,node_label,node_size,node_nudge,taxa_size,taxa_italic,taxa_align,taxa_offset,xmax,reverse_x,plot_title){
  ...
  if(Rboretum::isPhylo(tree,check_rooted = TRUE)){
    ...
    stop("'tree' must be either a rooted phylo object or a named, rooted, mulitPhlyo object for batch_supportTreePlotter")
  }
  
  clade_compare <- Rboretum
  ...
  if(missing(node_color)){
    node_color <- "red"
  } else if(has_error(silent=TRUE,expr=grDevices::col2rgb(node_color))){
    node_color <- "red"
  }
  ...
  if(!clade_support){
    ...
    return_tree <- return_tree + 
      ...
      scale_size_identity() +
      scale_discrete_manual(aesthetics = c('color'),limits = color_range,values = viridisLite::viridis(length(color_range)),name = "Trees with Split") +
