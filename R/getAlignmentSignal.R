...
getAlignmentSignal <- function(alignment_path,species_info,use_gaps,alignment_name,prefix,suffix,existing_signal){
  ...
  if(alignment_count == 1){
    ...
    splits_df = purrr::map(.x=1:alignment_count,.f=function(x){user_python,rb_run_align_splits(rb_lib_dir,alignment_path[x],species_info[x],gap_list[x],alignment_name[x])}) %>% do.call(rbind, .) %>%
      mutate_if(is.list,as.character) %>%
      select(-index) %>%
      mutate_all(~na_if(., 'NaN')) %>%
      rowwise() %>%
