...
def windowsPool(pool_cpu,pos_list,alignment,spp,base):
    ...
    var_getSitePatterns=partial(getSitePatterns_Windows, pruned_alignment = alignment,bases = base, spp_list = spp)
    pattern_results = pool.map(var_getSitePatterns, pos_list)
    pool.close()
    return(pattern_results)

def getSitePatterns_Windows(pos,pruned_alignment,bases,spp_list):
...
def patternProcessor(path_to_align,spp_info,use_gaps,align_name):
    ...
    if os.name == 'nt':
        pattern_results = windowsPool(pool_cpu,alignment_positions,pruned_alignment,spp_list,bases)
    else:
